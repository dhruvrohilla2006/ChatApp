...
const Profile = () => {
  ...
  const handleImageUpload = async (e) => {
    const file = await e.target.files[0]; // Get the selected file
    if (file) {
      const objectUrl = URL.createObjectURL(file); // Create a temporary URL for the file
      const base64 = await imageToBase64(file);
    ;

      setImagePreviewUrl(objectUrl); // Store the URL in state
  ...
  useEffect(() => {
    return () => {
      if (ImagePreviewUrl) {
        URL.revokeObjectURL(ImagePreviewUrl); // Revoke the URL to free memory
      }
    };
  }, [ImagePreviewUrl]);
  return (
        ...
        <div className="bg-base-300 rounded-xl p-6 space-y-8">
          ...
          <div className="flex flex-col items-center gap-4">
            <div className="relative">
              <img
                src={authUser.profilePic || ImagePreviewUrl || "/avatar.png"}
                alt="Profile"
